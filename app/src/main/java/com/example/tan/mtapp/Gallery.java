...
public class Gallery {
    ...
    private void setUpIntent(Intent PICKPictureIntent){
        File photoFile = StaticClass.createImageFile(context, dirName, imageName, imageType);
        if (photoFile != null) {
...
//        bitmap.recycle();
    ...
    public Bitmap resizeAndGetCameraBitmap(int imageHeight) {
        try {
            ...
            cameraBitmap = StaticClass.decodeFile(new File(cameraBitmapPath), imageHeight);
            if (cameraBitmap != null) {
                if (isCorrectOrientationRequired) {
                    cameraBitmap = StaticClass.rotateBitmap(cameraBitmap, StaticClass.getImageRotation(cameraBitmapPath));
                }
                StaticClass.saveBitmap(cameraBitmap, cameraBitmapPath, imageType, compression);
            }
            return cameraBitmap;
        } catch (Exception e) {
            return null;
    ...
    public static class Builder {
        ...
        public Gallery build(android.support.v4.app.Fragment fragment) {
